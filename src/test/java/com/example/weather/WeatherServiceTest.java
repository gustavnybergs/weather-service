...
public class WeatherServiceTest {
    ...
    @Test
    void testFetchCurrentWeatherAtSpecificLocation_notFound() {
        String location = "UnknownCity";

        Map<String, Object> apiResponse = new HashMap<>();
        apiResponse.put("results", Collections.emptyList());

        when(restTemplate.getForObject("https://geocoding-api.open-meteo.com/v1/search?name=" + location,
                Map.class
        )).thenReturn(apiResponse);

        Map<String, Object> response = weatherService.fetchCurrentWeatherAtSpecificLocation(location);
        assertNull(response);
    }
}
